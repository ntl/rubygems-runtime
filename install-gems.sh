...
for executable in "${executables[@]}"; do
  relative_bundler_setup="$(get_relative_path "$gems_exec_dir" gems/lib/bundler/setup.rb)"
...
#!/usr/bin/env ruby
# Generated by $0
require_relative '$relative_bundler_setup'
...
echo
echo "Generating bundler/setup"
echo "- - -"

bundler_setup_rb="gems/lib/bundler/setup.rb"

mkdir -p "$(dirname "$bundler_setup_rb")"

ruby -rpp -rpathname -rrubygems >"$bundler_setup_rb" <<RUBY
gem_home = '$GEM_HOME'
...
bundler_setup_dir = File.expand_path(File.dirname('$bundler_setup_rb'), __dir__)
...
[$(printf '"%s", ' "${gemspecs[@]}")].each do |gemspec|
  ...
  gem_specification.require_paths.each do |require_path|
    ...
    relative_require_path = Pathname(require_path).relative_path_from(bundler_setup_dir).to_s
...
if not gem_load_paths.empty?
  relative_gem_path_base = Pathname(File.expand_path("gems/#{RUBY_ENGINE}/$ruby_api_version")).relative_path_from(bundler_setup_dir)
